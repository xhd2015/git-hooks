...
- Stdout confirms hook added (`Added pre-commit hook: global-hook`).
...
- Managed hook exists at `$XDG_CONFIG_HOME/.git-hooks/pre-commit.d/global-hook`.
- Local `.git/hooks/pre-commit` and `.git/hooks/pre-push` are not created.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !strings.Contains(resp.AddOutput, "Added pre-commit hook: global-hook") {
	...
	managedPath := filepath.Join(globalManagedDir(req, "pre-commit"), "global-hook")
	if _, err := os.Stat(managedPath); err != nil {
		t.Fatalf("global managed hook missing at %s: %v", managedPath, err)
	}

	localPreCommit := localDispatcherPath(req, "pre-commit")
	if _, err := os.Stat(localPreCommit); !os.IsNotExist(err) {
		t.Fatalf("local pre-commit dispatcher should not exist at %s: %v", localPreCommit, err)
	}
	localPrePush := localDispatcherPath(req, "pre-push")
	if _, err := os.Stat(localPrePush); !os.IsNotExist(err) {
		t.Fatalf("local pre-push dispatcher should not exist at %s: %v", localPrePush, err)
	}
}
```